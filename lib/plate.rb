...
# Plate class
class Plate
  attr_reader :grid, :plate_size, :samples, :reagents, :num_of_replicates,
              :count_of_cells, :results, :all_combination
  ...
  def initialize(plate_size, samples, reagents, num_of_replicates)
    ...
    @grid = generate_empty_layout
    @all_combination = expanded_combination_result_flatten
  end
  ...
  def expanded_combination_result_flatten
    @all_combination = samples.map.with_index do |sample_list, index|
