# require_relative './lib/pate'
require 'debug'

# Class for create layout of combinations
class Placement
  attr_reader :results, :plate_size, :samples, :reagents, :num_of_replicates
  ...
  def calc_plates
    (calc_cells / plate_size) + ((calc_cells % plate_size).zero? ? 0 : 1)
  end
  ...
  def fill_layout(flatten_list = [])
    @results = generate_empty_layout.each do |plate|
      plate.map do |rows|
...
result = Placement.new(96,
                       ...
                       [15, 21, 7])
# создаем Доску и заполняем значениями пустыми
# у доски должен быть метод что вернет массив с нужным кол-вом влеженных досок
